...
class Player(Base):
    ...
    jersey_name = Column(String, unique=True, nullable=False)
    jersey_number = Column(Numeric, unique=True, nullable=False)
    photo = Column(String, nullable=True)
    created_at = Column(TIMESTAMP(timezone=True), nullable=False, server_default=text("now()"))
    updated_at = Column(TIMESTAMP(timezone=True), nullable=False, server_default=text("now()"))

    # Relationship with the Income table
    incomes = relationship('Income', back_populates='player')

class Income(Base):
    __tablename__ = 'income'
    id = Column(UUID(as_uuid=True), primary_key=True, nullable=False, default=uuid.uuid4)
    name = Column(String, nullable=False)  # Adding the name column to store the player's name
    ...
    player = relationship('Player', back_populates='incomes')
class Expense(Base):
